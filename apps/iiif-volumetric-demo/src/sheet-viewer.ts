...
//
// The synthetic cube volume comes from the IIIF Volumetric Server
// (/api -> /volumes/{id}/raw); the headline T1 is served alongside the
// mesh fixtures from @niivue/dev-images (/volumes/* and /meshes/*),
...
// The non-synthetic cells all load the same T1, served by the
// @niivue/dev-images vite plugin at `/volumes/<file>` so it works even
// if the IIIF server's fixtures dir does not contain it.
const FEATURED_T1_ID = 'chris_t1'
const FEATURED_T1_URL = '/volumes/chris_t1.nii.gz'
...
async function fetchVolumeIds(): Promise<string[]> {
  ...
  // Cell 0 is the synthetic cube + symmetric overlay (the headline of this
  // demo) — generated by the IIIF server. The remaining cells all load
  // the same T1 from @niivue/dev-images so the variety on screen is
  // "9 colormaps over one anatomy" rather than "9 anatomies".
  const synthetic = list.find((v) => v.id === SYNTHETIC_VOLUME_ID)
  const ids: string[] = []
  if (synthetic) ids.push(synthetic.id)
  while (ids.length < CELL_COUNT) ids.push(FEATURED_T1_ID)
  return ids
}

function urlForVolumeId(id: string): string {
  // FEATURED_T1_ID is served by the dev-images vite plugin at a
  // flat path; everything else is proxied to the IIIF server.
  if (id === FEATURED_T1_ID) return FEATURED_T1_URL
  return `/volumes/${encodeURIComponent(id)}/raw`
}
