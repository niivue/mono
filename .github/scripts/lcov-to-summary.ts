...
for (const rawLine of lcov.split('\n')) {
  ...
  switch (key) {
    ...
    case 'LH':
      ...
      break
    case 'FNF':
      current.fnFound = value
      break
    case 'FNH':
      current.fnHit = value
      break
    case 'BRF':
      current.brFound = value
      break
    case 'BRH':
      current.brHit = value
      break
...
lines.push('| --- | ---: | ---: | ---: |')
lines.push(
  `| Lines | ${totals.linesHit} | ${totals.linesFound} | ${fmtPct(linesPct)} |`,
)
lines.push(
  `| Functions | ${totals.fnHit} | ${totals.fnFound} | ${fmtPct(fnPct)} |`,
)
if (totals.brFound > 0) {
  lines.push(
    `| Branches | ${totals.brHit} | ${totals.brFound} | ${fmtPct(brPct)} |`,
  )
}
lines.push('')
