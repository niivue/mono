...
nx-mono-repo-test/
├── apps/
│   ├── ui-a/            TypeScript app (Bun) → depends on ts-package-d
│   ├── ui-b/            TypeScript app (Bun)
│   └── api-z/           Python API (Pixi)   → depends on py-package-c
├── packages/
│   ├── ts-package-d/    TypeScript library (Bun)
│   └── py-package-c/    Python library (Pixi)
├── nx-tools/
...
**Bun** is the JavaScript runtime and package manager. The three TypeScript projects are registered as [Bun workspaces](https://bun.sh/docs/install/workspaces) in the root `package.json`, sharing a single lockfile and hoisted `node_modules`.
...
```
ui-a ──→ ts-package-d     (detected from package.json / implicitDependencies)
api-z ──→ py-package-c    (detected automatically by the Pixi graph plugin)
ui-b (independent)
```
...
Pixi ignores `[tool.*]` sections, so this doesn't interfere with conda dependency resolution. The plugin creates a graph edge for each listed project. This means Python projects get the same automatic dependency detection as TypeScript projects.
...
| `feat:` | Minor (0.1.0 → 0.2.0) | `feat: add CSV export to ts-package-d` |
...
3. **Git** — stages the changed files, creates a `chore(release): publish` commit, and tags each package (e.g. `ts-package-d@0.4.1`, `py-package-c@0.4.1`)

#### Example: what this looks like in practice

After making commits to both packages:

```
848cd80 fix: improve subtract docstring with argument descriptions     (py-package-c)
d37b0ad feat: add divide and clamp utility functions to py-package-c   (py-package-c)
076dcad fix: handle whitespace-only names in greet function            (ts-package-d)
3d0c62a feat: add divide and clamp utility functions to ts-package-d   (ts-package-d)
```

Running `bunx nx release --skip-publish` produces:

```
ts-package-d 🏷️  Resolved the current version as 0.3.0 from git tag "ts-package-d@0.3.0"
ts-package-d 📄 Resolved the specifier as "minor" using git history
ts-package-d ❓ Applied semver relative bump "minor" to get new version 0.4.0
ts-package-d ✍️  New version 0.4.0 written to packages/ts-package-d/package.json

py-package-c 🏷️  Resolved the current version as 0.3.0 from git tag "py-package-c@0.3.0"
py-package-c 📄 Resolved the specifier as "minor" using git history
py-package-c ❓ Applied semver relative bump "minor" to get new version 0.4.0
py-package-c ✍️  New version 0.4.0 written to packages/py-package-c/pyproject.toml
```

Each package gets its own changelog. For ts-package-d:
...
- add divide and clamp utility functions to ts-package-d

### 🩹 Fixes

- handle whitespace-only names in greet function
```

Key behaviors:
- Each package is versioned **independently** — they can be on different versions
- Only commits touching files in a package's directory affect that package's version
- The `feat:` commits trigger a **minor** bump; if only `fix:` commits exist, it's a **patch** bump
- `docs:`, `chore:`, `refactor:` commits are excluded from the version calculation but may appear in the changelog
- Both `package.json` (TS) and `pyproject.toml` (Python) manifests are updated automatically
...
- **releaseTag.pattern** — tags follow `{projectName}@{version}` format (e.g. `ts-package-d@0.4.1`)
...
cd apps/api-z && pixi install && cd ../..
cd packages/py-package-c && pixi install && cd ../..
```
...
```bash
bunx nx run ui-a:dev
bunx nx run ui-a:build
bunx nx run ui-a:lint
bunx nx run ui-a:format
bunx nx run ui-a:typecheck
bunx nx run ui-a:test

bunx nx run api-z:dev
bunx nx run api-z:lint
bunx nx run api-z:format
bunx nx run api-z:test
```
