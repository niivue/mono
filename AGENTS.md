...
- **WASM workers require `optimizeDeps.exclude`.** Vite's dep prebundler trips on dynamic-import WASM workers. Any Vite app importing `@niivue/niimath` (directly or via `@niivue/nv-ext-niimath`) must exclude `'@niivue/niimath'` and `'@niivue/nv-ext-niimath'`. Apps importing `@niivue/dcm2niix` (directly or via `@niivue/nv-ext-dcm2niix`) must likewise exclude `'@niivue/dcm2niix'` and `'@niivue/nv-ext-dcm2niix'`. Apps that also use `@niivue/nv-ext-brain2print`'s quality mesh path must exclude `'@itk-wasm/cuberille'`, `'@itk-wasm/mesh-filters'`, and `'itk-wasm'`. See `apps/demo-ext-niimath/vite.config.js` (niimath only), `apps/demo-ext-dcm2niix/vite.config.js` (dcm2niix only), and `apps/demo-ext-brain2print/vite.config.js` (full 7-entry set: niimath, dcm2niix, and itk-wasm trio).
