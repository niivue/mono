...
function emitJs(api: ApiDescriptor): string {
  ...
  lines.push('')
  lines.push('  // ipywidgets does NOT replay msg:custom messages received')
  lines.push('  // before our listener is registered. If Python fires a')
  lines.push('  // command (e.g. `wait_ready()`) during JS cold-mount — which')
  lines.push("  // can take seconds for the 1.1 MB bundle — that message is")
  lines.push('  // dropped and the awaiting Future hangs forever.')
  lines.push('  //')
  lines.push("  // Register cmdHandler BEFORE any async work. Commands that")
  lines.push('  // need `nv` await `mountedPromise`, which resolves once')
  lines.push('  // attachToCanvas completes. Effect: messages received')
  lines.push('  // during mount are queued by the Promise machinery and')
  lines.push('  // processed in order as soon as `nv` is ready.')
  lines.push('  let nv = null')
  lines.push('  let mountedResolve')
  lines.push('  const mountedPromise = new Promise((r) => {')
  lines.push('    mountedResolve = r')
  lines.push('  })')
  lines.push('')
  ...
  lines.push('  //')
  lines.push('  // Caps:')
  lines.push("  //  - Typed arrays > 1024 elements are dropped (`null`). NiiVue")
  lines.push('  //    events like `volumeLoaded` carry the full voxel buffer in')
  lines.push("  //    their detail; serializing 50+ MB across the WebSocket")
  lines.push('  //    blocks long enough for JupyterLab to reconnect.')
  lines.push('  //  - `seen` WeakSet prevents traversal cycles (NVImage holds')
  lines.push('  //    back-refs into the controller).')
  ...
  lines.push('  }')
  lines.push('')
  lines.push('  // Command handler. Registered before mount; commands that')
  lines.push('  // need `nv` await `mountedPromise`. `__ready__` also awaits')
  lines.push("  // mount so Python's wait_ready() unblocks at the right moment.")
  ...
  lines.push('')
  lines.push('  // Wait one animation frame before NiiVue attaches so the')
  lines.push('  // browser has measured the parent container. Without this,')
  lines.push("  // `canvas.clientWidth` is 0/1 px during init and NiiVue's")
  lines.push("  // slice-layout math produces negative viewport bounds.")
  lines.push('  await new Promise((r) => requestAnimationFrame(() => r()))')
  lines.push('')
  lines.push('  // Build constructor opts from non-null traitlets — these')
  ...
  lines.push('  }')
  lines.push('')
  lines.push('  // Construct NiiVue and attach.')
  lines.push('  nv = new NiiVue(opts)')
  lines.push('  await nv.attachToCanvas(canvas)')
  lines.push('  mountedResolve()')
  lines.push('')
  lines.push('  // Seed Python with NiiVue\'s actual current values. This')
  ...
  lines.push('  }')
  lines.push('')
  lines.push('  // Cleanup on widget unmount.')
  lines.push('  return () => {')
