...
- **WASM workers require `optimizeDeps.exclude`.** Vite's dep prebundler trips on dynamic-import WASM workers. Any Vite app importing `@niivue/niimath` (directly or via `@niivue/nv-ext-niimath`) must exclude `'@niivue/niimath'` and `'@niivue/nv-ext-niimath'`. Apps that also use `@niivue/nv-ext-brain2print`'s quality mesh path must exclude `'@itk-wasm/cuberille'`, `'@itk-wasm/mesh-filters'`, and `'itk-wasm'`. See `apps/demo-ext-niimath/vite.config.js` (niimath only) and `apps/demo-ext-brain2print/vite.config.js` (full set).
