...
| Orthogonal slices in the 3D render | ✅ | `volumeRenderMode = VOLUME_RENDER_MODE.SLICES` (both backends). The old package reached the same view through a negative `setVolumeRenderIllumination` (`fragRenderSliceShader`, niivue issue 679); this is an explicit enum member. Two deliberate differences: the up-to-three plane hits are COMPOSITED front to back with air transparent (0.6 drew only the nearest hit, forced opaque, so black air hid the planes behind it), and overlays, PAQD and the drawing are sampled ON the planes rather than ray-marched volumetrically over them — which is what keeps it at three samples per ray with layers loaded. Clip planes are ignored, as in 0.6. Demo: `examples/vox.rendermode.html`. Follow-ups: overlay outlines, drawing rim opacity, V1 fiber lines, `volumeIsColormapAlphaOn2D`, tricubic sampling on the planes |
