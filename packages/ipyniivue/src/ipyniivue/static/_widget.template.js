...
// Generated at: 2026-04-26T17:48:03.673Z
...
async function render({ model, el }) {
  ...
  // ipywidgets does NOT replay msg:custom messages received
  // before our listener is registered. If Python fires a
  // command (e.g. `wait_ready()`) during JS cold-mount — which
  // can take seconds for the 1.1 MB bundle — that message is
  // dropped and the awaiting Future hangs forever.
  //
  // Register cmdHandler BEFORE any async work. Commands that
  // need `nv` await `mountedPromise`, which resolves once
  // attachToCanvas completes. Effect: messages received
  // during mount are queued by the Promise machinery and
  // processed in order as soon as `nv` is ready.
  let nv = null
  let mountedResolve
  ...
  // Command handler. Registered before mount; commands that
  // need `nv` await `mountedPromise`. `__ready__` also awaits
  // mount so Python's wait_ready() unblocks at the right moment.
  ...
  // Build constructor opts from non-null traitlets — these
  ...
  // Construct NiiVue and attach.
  nv = new NiiVue(opts)
  await nv.attachToCanvas(canvas)
  mountedResolve()

  // Seed Python with NiiVue's actual current values. This
