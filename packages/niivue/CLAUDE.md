...
npm run build        # Library build to ./dist (vite.config.lib.ts, ES + UMD)
npm run build:examples # Examples site build (vite.config.examples.ts)
npm run deploy       # Production examples-site build (used by GitHub Pages)
npm run lint         # ESLint check
npm run lint:fix     # ESLint auto-fix
npm run typecheck    # TypeScript type checking (tsc --noEmit)
```

No test suite — verify via interactive demos. **Before committing**, run `npm run lint:fix && npm run typecheck`.

`npm run dev` uses a Vite plugin (`vite.config.dev.js`) to redirect `import '../dist/niivuegpu.mjs'` to source for HMR. Library packaging: `dist/niivuegpu.js` (both backends), `dist/niivuegpu.webgpu.js`, `dist/niivuegpu.webgl2.js`, exported as `niivuegpu`, `niivuegpu/webgpu`, `niivuegpu/webgl2`.
...
| `NVDocument.ts` | Document save/load (NVD format v5, CBOR-encoded) |
...
**ESLint:** No semicolons, 2-space indent, strict equality (`===`), unused params prefixed with `_`.

**TypeScript:** Strict mode, ESNext target, bundler module resolution. `@webgpu/types` for GPU APIs.

**Logging:** Use `import { log } from '@/logger'` instead of `console.*`. Levels: debug/info/warn/error/fatal/silent. `NiiVueOptions.logLevel` is typed as `LogLevel` (same union, re-exported from `src/index.ts`). When `log.level === 'debug'`, views render a `'WebGPU'`/`'WebGL2'` backend badge in the corner (hidden otherwise).

### Module Naming

- **Public modules** (NV-prefixed, PascalCase): `NVModel.ts`, `NVMesh.ts`, `NVVolume.ts` — cross-directory imports
- **Internal modules** (lowercase): `mesh.ts`, `font.ts`, `orient.ts` — folder-local only

### Import Path Aliases

Cross-directory imports use `@/` alias. Relative imports (`./mesh`) only within same directory.

## Coordinate Systems

Two distinct fractional [0..1] spaces — do not conflate:

- **Scene fraction**: [0,1]³ within scene AABB. Used for `crosshairPos`. Conversion: `NVModel.scene2mm()`/`mm2scene()`.
- **Volume texture fraction**: [0,1]³ within a single volume's 3D texture. Per-image: `NVImage.frac2mm` mat4. Model caches background volume's matrices as `NVModel.tex2mm`/`mm2tex`.

All overlay volumes are resliced to match the background volume's grid, so `tex2mm`/`mm2tex` apply to all loaded overlays.

**MM space** is the common ground. Depth picker → mm → scene fraction for `crosshairPos`. `getSliceTexFrac()` converts scene fraction → texture fraction.
...
All format readers use Vite's `import.meta.glob` for automatic discovery. To add a new reader: create a module in the appropriate `readers/` directory, export `extensions` array and `read` function. Auto-registered at build time.
...
Exactly one of `mz3`, `trx`, `jcon` is non-null per mesh. Source data is immutable; only derived GPU arrays change.

## Mesh Layers (Scalar Overlays)

CPU-composited scalar overlays on meshes. `perVertexColors` (nullable `Uint32Array`, packed ABGR) preserves file-provided vertex colors for recompositing. Layer readers in `mesh/layers/readers/`. Layers only apply to `kind === 'mesh'`.
...
`NVWorker` (`workers/NVWorker.ts`) provides promise-based worker bridge with message-ID tracking and transferable support. Use `?worker&inline` for imports (blob URL). Strip non-cloneable properties via `JSON.parse(JSON.stringify())` before `postMessage`.
